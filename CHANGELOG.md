...
- **16 new assertions.** 11 snapshot integration tests (above- and below-threshold impossible-output proofs that the fast path was used, a re-arm cycle covering stale-snapshot fallback and refreshed-snapshot re-arm, mtime and size freshness-gate mismatches, malformed-JSON fallback, and a cross-platform crash guard) plus 5 writer-script tests (token math for both waterfall paths, null-source stale-snapshot deletion, `safe_sid` parity for regex-valid and traversal inputs). Total assertions: 61.
