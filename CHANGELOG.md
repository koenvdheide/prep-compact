...
- **12 new assertions.** 8 snapshot integration tests (above- and below-threshold impossible-output proofs that the fast path was used, re-arm flow via stale snapshot + fallback, freshness-gate mismatch, and a cross-platform crash guard) plus 4 writer-script tests (token math for both waterfall paths, null-source stale-snapshot deletion, traversal `session_id` sanitization). Total assertions: 57.
